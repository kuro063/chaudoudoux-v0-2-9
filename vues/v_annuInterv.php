...
<!-- Division pour le contenu principal -->
    ...
    <table class="sortable zebre" id="listeInterv">
        ...
        echo '<span style="font-size: 25px" ><strong>- PRESTATION MÉNAGE</strong> - Nombre d\'heures par semaine : '.round($nbhTot,2).' h</span>';
          ...
          <?php $i=0;
            ...
            foreach ($lesInterv as $uneInterv){
              ...
              if ($numFam!=9999 && $uneInterv['type_Prestations'] == 'Garde d\'enfants'){
                ...
                if ($dateFinstring=='00/00/0000'){
                $dateFinstring="Indeterminé";}
                ...
                if ($idADH!='MAND'){
                  echo '<tr>';
                    echo '<td style="display: none;">'.$addrCan.'</td>
                          <td style="display: none;">'.$addrFam.'</td>';
                  if (lireDonneeUrl('num')!=99999){ 
                    echo '<td>'.$idSal.'</td>
                          <td><a href="index.php?uc=annuSalarie&action=voirDetailSalarie&num='.$numSal.'">'.$nomSal.'</a></td>
                          <td>'.$nomJF.'</td>';} 
                    echo '<td><a href="index.php?uc=annuFamille&action=voirDetailFamille&num='.$numFam.'">'.$nomFam.'</td>
                          <td>';
                    echo $presta." ".$libelleADH;
                    echo'</td>
                          <td>'.$dateDebstring.'</td>
                          <td>'.$dateFinstring.'</td>
                          <td sorttable_customkey="'.ckJour($jour).$hDeb.$hFin.'">';
                  echo $jour.' '.$hDeb.'-'.$hFin.'<br/>'.$modalites.'<td>';
                  $nbminFin=((int) substr($hFin,3,2));$nbminDeb=((int) substr($hDeb, 3,2));
                  if ($nbminFin==45){$nbminFin=0.75;}elseif($nbminFin==30){$nbminFin=0.5;}elseif($nbminFin==15){$nbminFin=0.25;}else{$nbminFin=0;}
                  if ($nbminDeb==45){$nbminDeb=0.75;}elseif($nbminDeb==30){$nbminDeb=0.5;}elseif($nbminDeb==15){$nbminDeb=0.25;}else{$nbminDeb=0;}
                  ...
                  echo '</td>
                        <td style="border-left: solid black 1px;border-right: solid black 1px; border-collapse : collapse"><img style="max-width:30px;" src="./styles/img/loading.gif"></img></td>';
                  echo '</td>
                        <td style="border-left: solid black 1px;border-right: solid black 1px; border-collapse : collapse"><img style="max-width:30px;" src="./styles/img/loading.gif"></img></td>';
                  echo '</td>
                        <td><a href="index.php?uc=interventions&amp;action=modifInterv&amp;num='.$numSal.'&amp;numFam='.$numFam.'&amp;idADH='.$idADH.'&amp;idPresta='.$idPresta.'&amp;hDeb='.$hDeburl.'&amp;dateDeb='.$dateDeburl.'&amp;dateFin='.$dateFinurl.'&amp;jour='.$jour.'&amp;hFin='.$hFinurl.'&amp;freq='.$freq.'">Modifier</a>';
                  echo '</td>
                        <td><a href="index.php?uc=interventions&amp;action=archiverIntervention&amp;num='.$numSal.'&amp;numFam='.$numFam.'&amp;idADH='.$idADH.'&amp;idPresta='.$idPresta.'&amp;hDeb='.$hDeburl.'&amp;dateDeb='.$dateDeburl.'&amp;jour='.$jour.'&amp;hFin='.$hFinurl.'">Terminé</a></td></tr>';
                  $i++;
                }
              }
            }
        echo '<span style="font-size: 25px" ><strong>- PRESTATION GARDE D\'ENFANTS UNIQUEMENT EN PERIODE SCOLAIRE</strong> - Nombre d\'heures par semaine : '.round($nbhTot,2).' h</span>';
        ?><tfoot><tr><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td><strong style="float:right"><?php echo 'Total Prestataire : '.round($nbhTot,2); ?></tfoot></strong></td></tr></tfoot></table><br/><br/>
                <table class="sortable zebre" id="listeInterv">
        <thead>   <tr class="btn-secondary">
            <?php if (lireDonneeUrl('num')!=99999){?><th>Numéro du salarié</th><th>Nom du Salarié</th><th>Nom de jeune fille</th><?php }?><th>Nom de la famille</th><th>Type de prestation</th><th>Date de début</th><th>Date de fin</th><th>Jour / horaires / modalités</th><th>HRES/SEM</th><th>Modifier</th><th>Terminé?</th>
