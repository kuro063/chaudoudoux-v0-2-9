...
<?php 
/* *************************************************
  Page de matching pour une famille et de ses matchs
**************************************************** */
if(lireDonneeUrl('action')=='voirIntervenantsDispo'){ ?>
  ...
  <style>            
    ...
    //   $demandes=$pdoChaudoudoux->obtenirBesoins($numFamille);
    //   foreach ($demandes as $key => $uneDemande)
    //       {
    //         $jourF=$uneDemande['jour'];
    //         $hDebF=$uneDemande['heureDebut'];
    //         $hFinF=$uneDemande['heureFin'];
    //       }

                        
    $lesIntervenantsDispo=$matching->obtenirIntervenantsDispo($numFamille);
                                                    

    echo "<legend>TABLEAU DES INTERVENANTS DISPONIBLES</legend>";
    echo "<input type='checkbox' id='choixTabloInterv' onchange='tableauIntervDispo()' checked />Tableau des intervenants disponible";

    echo "<input type='checkbox' id='choixTabloIntervRchCompl' onchange='tableauIntervDispoRCompl()' checked  />Tableau des intervenants avec l'option recherche heure compléments";

    echo "<table id='tabloInterv' style='width:100%;'>";
      echo "<thead>";
      //  echo "<th> Id de la disponibilité </th>";
        ...
        echo "<th> Supprimer la disponibilité </th>";
      echo "</thead>";
    echo "<tbody>";
    foreach ($lesIntervenantsDispo as $key => $unIntervenantDispo)
      {                           
        $jour=$unIntervenantDispo['jour'];
        ...
        $hDeb=$unIntervenantDispo['heureDebut'];
        $hFin=$unIntervenantDispo['heureFin'];
        $frequence=$unIntervenantDispo['frequence'];
        $activite=$unIntervenantDispo['activite'];
        $id=$unIntervenantDispo['id'];

        echo "<tr>";
          echo "<td class='dispo'><a href='index.php?uc=annuSalarie&action=voirDetailSalarie&amp;num=$numInterv'</a>".$numSalarie."</td>";
          echo "<td class='dispo'>".$nomCandidat."</td>";
          echo "<td class='dispo'>".$activite." </td>";                             
          echo "<td class='dispo'> ".$jour." - ".$hDeb." à ".$hFin." <br/><br></td>";                         
          echo "<td class='dispo'> Une semaine sur ".$frequence."  </td>";
          echo '<td class="dispo"> <a href="index.php?uc=interventions&amp;action=supprimerDispoInterv&amp;numDispo='.$id.'">Supprimer</a> </td>';
        echo '</tr>';
  }
                      
                    
  $lesIntervenantsRechCompl=$matching->obtenirIntervenantsRechCompl($numFamille);

  foreach ($lesIntervenantsRechCompl as $unIntervenantRechCompl) {

    $jour=$unIntervenantRechCompl['jour'];
    ...
    $hDeb=$unIntervenantRechCompl['heureDebut'];
    $hFin=$unIntervenantRechCompl['heureFin'];
    $frequence=$unIntervenantRechCompl['frequence'];
    $activite=$unIntervenantRechCompl['activite'];
    $id=$unIntervenantRechCompl['id'];

    echo "<td class='rchCompl'><a href='index.php?uc=annuSalarie&action=voirDetailSalarie&amp;num=$numInterv'</a>".$numSalarie."</td>";
    echo "<td class='rchCompl'>".$nomCandidat."</td>";
    echo "<td class='rchCompl'>".$activite."</td>";                   
    echo "<td class='rchCompl'>".$jour." - ".$hDeb." à ".$hFin."<br><br></td>";
    echo "<td class='rchCompl'> Une semaine sur ".$frequence." </td>";
    echo '<td class="rchCompl"><a href="index.php?uc=interventions&amp;action=supprimerDispoInterv&amp;numDispo='.$id.'">Supprimer</a> </td>';
  ...
  function tableauIntervDispo(){
    // let tableauInterv=document.querySelectorAll('.dispo');
    // tableauInterv.hidden=!tableauInterv.hidden;
    var cells = document.getElementsByClassName("dispo");
  ...
  function tableauIntervDispoRCompl(){
    //let tableauInterv=document.querySelectorAll('.rchCompl');
    //tableauInterv.hidden=!tableauInterv.hidden;
    var cells = document.getElementsByClassName("rchCompl");
      for (var i = 0; i < cells.length; i++) {
        if (cells[i].style.display === "none") {
          cells[i].style.display = "table-cell";
        } else {
...
********************************************************** */
if(lireDonneeUrl('action')=='voirMatchingFamilleGE' || lireDonneeUrl('action')=='voirMatchingFamilleMenage'){
  ?>
  <style>       
    table,td,th,tr{
      border: 1px solid black;
  ...
  <script>
    /*
      ...
      mais dont aucun d'entre eux ne peux matcher à cause des paramètres d'affinage (ex : besoin de voiture)
    */
  ...
  if(lireDonneeUrl('action')=='voirMatchingFamilleGE'){
    $tousIntervenantsDispo = $matching->obtenirAllnumFamilleBesoinsGE();
  }
  elseif(lireDonneeUrl('action')=='voirMatchingFamilleMenage') {
  ...
  elseif(lireDonneeUrl('action')=='voirMatchingFamilleMenage'){
    ...
    // Form qui vérifie si l'utilisateur veut prendre en compte ou non la localisation (secteur et ville)
    // dans le matching
    if(lireDonneeUrl('action')=='voirMatchingFamilleGE'){
      echo "<form method='post' action='index.php?uc=annuFamille&action=voirMatchingFamilleGE'>";
      if(isset($ignoreSecteur) && $ignoreSecteur == "Ignorer la localisation"){ 
        echo "  <input type='submit' name='ignoreSecteur' class='btn btn-default' style='background-color:#FFB0B0' value='Ne plus ignorer la localisation'>";
      }
      else{
        echo"<input type='submit' name='ignoreSecteur' class='btn btn-default' style='background-color:#FFB0B0' value='Ignorer la localisation'>";
      }   
      echo "</form>";
    }

    if(lireDonneeUrl('action')=='voirMatchingFamilleMenage'){
      echo "<form method='post' action='index.php?uc=annuFamille&action=voirMatchingFamilleMenage'>";
      if(isset($ignoreSecteur) && $ignoreSecteur == "Ignorer la localisation"){ 
        echo "  <input type='submit' name='ignoreSecteur' class='btn btn-default' style='background-color:#FFB0B0' value='Ne plus ignorer la localisation'>";
      }
      else{
    ...
    // Lien qui permet d'acceder à la page des intervenants masqués du matching (par la table : antimatching)
    if(lireDonneeUrl('action')=='voirMatchingFamilleGE'){
      echo "<p><a href='index.php?uc=annuFamille&action=voirMatchingFamilleGEMasque'> Voir les intervenants masqués</a></p>";
    }
    elseif(lireDonneeUrl('action')=='voirMatchingFamilleMenage'){
      echo "<p><a href='index.php?uc=annuFamille&action=voirMatchingFamilleMenageMasque'> Voir les intervenants masqués</a></p>";
    }
  ...
  while($i<count($tousIntervenantsDispo)){
    ...
    $secteurFamille = $tousInfosFamille['secteur_Famille'];
    ...
    if($lesIntervenantsDispo!=null || $lesIntervenantsRechCompl!=null){
      ...
      echo "<table id='tableInterv$i'   class='tableInterv'  style='width:100%; '>";
        echo "<thead>";
          ...
          echo "<th> Fréquence de la prestation </th>";
          echo "<th> Masquer l'intervenant </th>";
        echo "</thead>";
      echo "<tbody>";
    }
    ...
    if($lesIntervenantsDispo!=null){ 
      ...
      foreach ($lesIntervenantsDispo as $key => $unIntervenantDispo){
        ...
        if(lireDonneeUrl('action')=='voirMatchingFamilleGE'){
          if($verifIntervMasque != null && $verifIntervMasque['GE']==1){
            $estMasque = true;
          }
        }
        elseif(lireDonneeUrl('action')=='voirMatchingFamilleMenage'){
          if($verifIntervMasque != null && $verifIntervMasque['menage']==1){
            $estMasque = true;
          }
        }
        ...
        $secteurCandidat = $tousInfosIntervCand['secteur_Candidats'];
        
        //On initialise les disponibilités
        $jour = '' ; 
        $hDeb = '' ; 
        $hFin = '' ; 
        $frequence = '' ; 
        ...
        if($ignoreSecteur != "Ignorer la localisation"){
          if(strtolower($ville_Famille) != "rennes"){
            if(strtolower($ville_Candidats) != strtolower($ville_Famille)){
              if($bgColor == '#ffffff'){
                $bgColor = '#e3e3e3';
              }
              elseif($bgColor == '#e3e3e3'){
            ...
            if($secteurFamille != null && $secteurFamille != "Aucun" && $secteurCandidat != null && $secteurCandidat != "Aucun"){
              if($secteurFamille != $secteurCandidat){
                if($bgColor == '#ffffff'){
                  $bgColor = '#e3e3e3';
                }
                elseif($bgColor == '#e3e3e3'){
        ...
        echo "<tr style='background-color:$bgColor;color:#000000;' class='ligneInterv'>";
          ...
          if(lireDonneeUrl('action')=='voirMatchingFamilleGE'){
            echo "<td class ='dispo'><a href='index.php?uc=annuFamille&action=masquerIntervGE&amp;numInterv=$numInterv&amp;numFamille=$numFamille'</a> Masquer </td>";
          }
          elseif(lireDonneeUrl('action')=='voirMatchingFamilleMenage'){
            echo "<td class ='dispo'><a href='index.php?uc=annuFamille&action=masquerIntervMenage&amp;numInterv=$numInterv&amp;numFamille=$numFamille'</a> Masquer </td>";
          }
        echo '</tr>';
      ...
      foreach ($lesIntervenantsRechCompl as $unIntervenantRechCompl) {
        ...
        foreach($tousBesoinsFamille as $unBesoinFamille){
          ...
          foreach($matchIntervenantDispo as $unMatch){
            ...
            if($jourF != 'sans importance'){
              ...
              $dispo = "<br/>".$dispo.$lastDispoRechCompl;
              $lastDispoRechCompl = $dispo;
            } 
          }
        ...
        $lastDispoRechCompl = "";  
         echo "<tr style='background-color:$bgColor;color:#000000;' class='ligneInterv'>";
          ...
          if(lireDonneeUrl('action')=='voirMatchingFamilleGE'){
            echo "<td class ='rchCompl'><a href='index.php?uc=annuFamille&action=masquerIntervGE&amp;numInterv=$numInterv&amp;numFamille=$numFamille'</a> Masquer </td>";
          }
          elseif(lireDonneeUrl('action')=='voirMatchingFamilleMenage'){
            echo "<td class ='rchCompl'><a href='index.php?uc=annuFamille&action=masquerIntervMenage&amp;numInterv=$numInterv&amp;numFamille=$numFamille'</a> Masquer </td>";
          }
        echo '</tr>';
        }
      }          
  ...
  function toggleTableDispo() {
    ...
    for(let i=0 ; i < nbTable ; i++){
      ...
      //Récupère les sous-titres des tables contenant les avertissements
