...
if($action=='voirTousFamilleBesoinGEFutur')							{echo 'besoins futurs pour la garde d\'enfants<p style="text-transform: lowercase" >(intervenant en place actuellement) </p>';
   echo ($nbFamilleGE);
  }
...
<table style='width:100%;border: solid 1px black;border-collapse:collapse;' class="sortable zebre">
  <thead>
    <tr>
      ...
      <th> Intervenant actuelle </th>
      <th> Date fin planning de l'intervenant </th>
    ...
    while($i<count($lesBesoinsFamillesGE)){
      ...
      $nomIntervenat=$lesBesoinsFamillesGE[$i]["nom_Candidats"];
      $dateFin=$lesBesoinsFamillesGE[$i]["dateFin_Proposer"];
      $dateApourvoir=$lesBesoinsFamillesGE[$i]["Date_aPourvoir_PGE"];
      $numFamille=$lesBesoinsFamillesGE[$i]['numero_Famille'];
      $numInterv=$lesBesoinsFamillesGE[$i]['numSalarie_Intervenants'];
      ...
      $dateFin=explode('-', $lesBesoinsFamillesGE[$i]["dateFin_Proposer"]);
